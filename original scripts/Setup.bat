...
:: TODO - when setup is finished, show an OK dialog with an overview of what was done please :)
...
if "%DEPLOY_TO%" == "" (
    ...
    if "!MSGBOX_RESULT!" == "Yes" (
        ...
        if not "%SKYRIM_MODS_FOLDER%" == "" (
            if exist "%SKYRIM_MODS_FOLDER%" (
                ...
                @REM set MSGBOX_MSG=^!MSGBOX_MSG!`n`n"%SKYRIM_MODS_FOLDER%\%MOD_NAME%"
    ...
    if "!CONFIGURING_DEPLOY!" == "true" (
        ...
        if "!DEPLOY_TO!" == "" (
            ...
            @REM set MSGBOX_MSG=^!MSGBOX_MSG!`n`n"%SKYRIM_FOLDER%"
            for /f "usebackq delims=" %%i in (`
                powershell -c "Add-Type -Assembly System.Windows.Forms; [System.Windows.Forms.MessageBox]::Show(@\"!\n!!MSGBOX_MSG!!\n!\"@, '%MSGBOX_TITLE%', 'YesNo').ToString()"
            `) do set MSGBOX_RESULT=%%i
            if "!MSGBOX_RESULT!" == "Yes" (
